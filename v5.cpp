...
multimap<int, pair<int, int>> join(multimap<int, pair<int, int>> adj, int one, int two)
{
  ...
  for (int x = 0; x < alpha.size(); x++)
  {
    ...
    {
      if (alpha[x].second == beta[y].second)
      { 
...
bool checkcut(multimap<int, pair<int, int>> cut, int a, int b)
{
  ...
  while (it != cut.end())
  {
    ...
    it++;
    if (it->first != a)
    { // if we reached here that would mean we did not find the value
      cout << "Node: " << a << "---- join ---- " << b << endl;
      value = true;
      break;
    }
...
tulip depth_search(multimap<int, pair<int, int>> adj, tulip one, int node, int bound, int lower)
{
  ...
  for (int x = 0; x < adj.size(); x++)
  {
    ...
    if (ref.first != 0)
    {
      if (checkcut(one.pcut, it->first, ref.second) == true)
      {
        ...
        else
        {
          ...
          one.cluster = addEdge(one.cluster,node,ref.second,j);
    ...
    if (it->first != node)
    { // if it's been cut they cannot join
...
int main()
{
  ...
  for (int x = 0; x < myGraphData.size(); x++)
  { // Iterate over lines
    for (int y = 0; y < myGraphData[x].size(); y++)
    { // Iterate over weights in a line
